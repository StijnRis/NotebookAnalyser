...
from notebook_log.notebook_activity_composite import NotebookActivityComposite
...
class NotebookCellActivityComposite(NotebookActivityComposite, NotebookCellActivity):
    ...
    def get_similarity_between_cell_states(self, time1: datetime, time2: datetime):
        id = self.get_cell_id()
        end_result = self.get_notebook_cell_content_at(id, time1)
        current_result = self.get_notebook_cell_content_at(id, time2)
    ...
    def get_overview(self, level=1) -> str:
        return (
            ...
            f"```python\n{self.get_notebook_cell_content_at(self.get_cell_id(), self.get_start_time())}\n```\n\n"
            f"{'#' * (level + 1)} Final state\n\n"
            f"```python\n{self.get_notebook_cell_content_at(self.get_cell_id(), self.get_end_time())}\n```\n\n"
        )
