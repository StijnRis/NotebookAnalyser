...
class ChatInteraction(ChatLog):
    ...
    def check_invariants(self):
        ...
        assert isinstance(
            self.messages[0], ChatMessageQuestion
        ), "First message should be a question"

        # Check if the second message is an answer
        assert isinstance(
            self.messages[1], ChatMessageAnswer
        ), "Second message should be an answer"
